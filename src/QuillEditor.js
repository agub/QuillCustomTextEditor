...
class QuillEditor extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			editorHtml: __ISMSIE__ ? "<p>&nbsp;</p>" : props.content,
			files: [],
		};
			...
			const compFunction = async () => {
				...
				if (compressState.success) {
					...
					storageRef
						...
						.then(async (snapshot) => {
							const downloadedURL = await storageRef
								.ref()
								.child("Articles/" + fileName)
								.getDownloadURL();
							if (this._isMounted) {
								let lastDigitURL = await downloadedURL.slice(
									-15,
									downloadedURL.length
								);
								this.props.listUpdatedURL(lastDigitURL);
							}
							...
							quill.insertEmbed(position, "image", downloadedURL);
